...
function inicio() {
  ...
  file = new URL("./arraylocalidades.php", window.location.href);

  // Lanza evento para comprobar la localidad
  localidadUsuario.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      // Parametros de petición al servidor
      let opcionesPeticion = {
        method: "POST",
        body: "localidadUsuario=" + localidadUsuario.value,
        headers: {
          "Content-type": "application/x-www-form-urlencoded",
        },
      };

      cargarContenido(file, opcionesPeticion);
    }
  });
}

/** Carga de datos del servidor */
async function cargarContenido(file, opcionesPeticion) {
  let resultado = document.getElementById("resultado");
  try {
    let respuesta = await fetch(file, opcionesPeticion);
    console.log(respuesta);
    if (respuesta.ok) {
      // Devuelve respuesta
      let datosRespuesta = await respuesta.text();
      // Tratamiento
      if (datosRespuesta == "S") {
