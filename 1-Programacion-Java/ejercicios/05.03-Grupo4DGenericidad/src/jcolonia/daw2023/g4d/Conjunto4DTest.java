...
class Conjunto4DTest {
	...
	private Conjunto4D<Grupo4DMejorado> conjunto;
	...
	void setUp() throws Exception {
		...
		conjunto = new Conjunto4D<Grupo4DMejorado>(capacidad);
	...
	void testExcepciónConstructorListaCapacidad1() throws Grupo4DException {
		...
		ex = assertThrows(Grupo4DException.class, () -> new Conjunto4D<Grupo4DMejorado>(capacidad),
...
//		assertEquals(grupo4DMejorado, conjunto.consultarElemento(2), "Tercera posición");
	...
	/**
	 * Comprueba que la ordenación de los grupos usando su media es correcta.
	 * 
	 * @throws Grupo4DException no se espera que salte excepción para este caso
	 * @see Conjunto4D#eliminarElemento(int)
	 */
	@RepeatedTest(value = 200, name = "→ {currentRepetition}/{totalRepetitions}")
	@DisplayName("Ordenación por media")
	void test() throws Grupo4DException {
		Random rnd;
		...
		for (int i = 1; i < conjunto.getNúmElementos(); i++) {
			final Grupo4DMejorado grupo2 = conjunto.consultarElemento(i);
			final Grupo4DMejorado grupoAnterior2 = conjunto.consultarElemento(i - 1);
			contador = i;
			assertTrue(grupo2.media() > grupoAnterior2.media(),
					() -> String.format("Grupo %d(media=%f) debe ser mayor que: %d(media=%f)", contador, grupo2.media(),
							contador - 1, grupoAnterior2.media()));
		}
