...
//		assertEquals(grupo4DMejorado, conjunto.consultarElemento(2), "Tercera posición");
	...
	void test() throws Grupo4DException {
		Random rnd;
		Conjunto4D conjunto;
		Grupo4DMejorado grupoAnterior, grupo;
		double a, b, c, d;
		int capacidadConjunto;

		capacidadConjunto = 3;
		rnd = new Random();
		conjunto = new Conjunto4D(capacidadConjunto);
...
//		conjunto.ordenar();
		...
		for (int i = 1; i < conjunto.getNúmElementos(); i++) {
			final Grupo4DMejorado grupo2 = (Grupo4DMejorado) conjunto.consultarElemento(i);
			final Grupo4DMejorado grupoAnterior2 = (Grupo4DMejorado) conjunto.consultarElemento(i - 1);
			contador = i;
